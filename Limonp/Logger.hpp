...
#define LogFatal(fmt, ...) {Limonp::Logger::LoggingF(Limonp::LL_FATAL, FILE_BASENAME, __LINE__, fmt, ## __VA_ARGS__); exit(1);}
...
static const char * LOG_FORMAT = "%s %s %s\n";
...
class Logger {
 public:
  static void Logging(size_t level, const string& msg, const char* fileName, int lineno) {
    ...
    fprintf(stderr, LOG_FORMAT, buf, LOG_LEVEL_ARRAY[level], msg.c_str());
  }
